        ...
        switch (actionName)
        {
            case "SyncPlayCreateGroup":
                ...
                if (executed.Result is ObjectResult { Value: { } resultObj })
                {
                    ...
                    if (groupIdProp?.GetValue(resultObj) is Guid createdGroupId)
                    {
                        ...
                        log.LogInformation(
                            "SyncPlayGroupFilter: tracked create user={UserId} group={GroupId}",
                            userId, createdGroupId);
                    }
                    else
                    {
                        log.LogWarning("SyncPlayGroupFilter: could not extract GroupId from CreateGroup result");
                    }
                }
                else
                {
                    log.LogWarning(
                        "SyncPlayGroupFilter: CreateGroup result was not ObjectResult, was {Type}",
                        executed.Result?.GetType().Name ?? "null");
                }
                break;
            ...
            case "SyncPlayJoinGroup":
                ...
                if (TryGetGroupIdFromArgs(ctx, out var joinedGroupId))
                {
                    ...
                    log.LogInformation(
                        "SyncPlayGroupFilter: tracked join user={UserId} group={GroupId}",
                        userId, joinedGroupId);
                }
                else
                {
