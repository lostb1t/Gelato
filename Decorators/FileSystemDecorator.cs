...
public sealed class FileSystemDecorator : IFileSystem
{
    ...
    private static bool IsSubPathOf(string path, string basePath)
    {
        ...
        return normalizedPath.StartsWith(normalizedBase, StringComparison.OrdinalIgnoreCase)
            || normalizedPath.TrimEnd(Path.DirectorySeparatorChar) == normalizedBase.TrimEnd(Path.DirectorySeparatorChar);
    }
    ...
    private IReadOnlyList<BaseItem> GetChildItemsFromDb(string directory)
    {
        ...
        foreach (var item in allItems)
        {
            ...
            if (!itemPath.StartsWith(normalizedDir + Path.DirectorySeparatorChar, StringComparison.OrdinalIgnoreCase))
                continue;
    ...
    public FileSystemMetadata GetDirectoryInfo(string path)
    {
        ...
        return new FileSystemMetadata { FullName = path, Exists = false, IsDirectory = true };
    }
    ...
    public void CreateShortcut(string shortcutPath, string target) => _inner.CreateShortcut(shortcutPath, target);
    public string MakeAbsolutePath(string folderPath, string filePath) => _inner.MakeAbsolutePath(folderPath, filePath);
    ...
    public bool ContainsSubPath(string parentPath, string path) => _inner.ContainsSubPath(parentPath, path);
    public string GetFileNameWithoutExtension(FileSystemMetadata info) => _inner.GetFileNameWithoutExtension(info);
    ...
    public void SetAttributes(string path, bool isHidden, bool readOnly) => _inner.SetAttributes(path, isHidden, readOnly);
}
